...
OLAPStatus OlapMeta::get(const int column_family_index, const std::string& key, std::string* value) {
    DorisMetrics::instance()->read_meta_requests_total.increment(1);
    ...
    DorisMetrics::instance()->read_meta_requests_duration_us.increment(duration_ns / 1000);
    if (s.IsNotFound()) {
...
OLAPStatus OlapMeta::put(const int column_family_index, const std::string& key, const std::string& value) {
    DorisMetrics::instance()->write_meta_requests_total.increment(1);
    rocksdb::ColumnFamilyHandle* handle = _handles[column_family_index];
    int64_t duration_ns = 0;
    rocksdb::Status s;
    {
    ...
    DorisMetrics::instance()->write_meta_requests_duration_us.increment(duration_ns / 1000);
    if (!s.ok()) {
