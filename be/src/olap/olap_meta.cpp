...
OLAPStatus OlapMeta::get(const int column_family_index, const std::string& key, std::string* value) {
    DorisMetrics::instance()->meta_read_requests_total.increment(1);
    ...
    DorisMetrics::instance()->meta_read_requests_duration_us.increment(duration_ns / 1000);
    if (s.IsNotFound()) {
...
OLAPStatus OlapMeta::put(const int column_family_index, const std::string& key, const std::string& value) {
    DorisMetrics::instance()->meta_write_requests_total.increment(1);
    rocksdb::ColumnFamilyHandle* handle = _handles[column_family_index];
    int64_t duration_ns = 0;
    rocksdb::Status s;
    {
    ...
    DorisMetrics::instance()->meta_write_requests_duration_us.increment(duration_ns / 1000);
    if (!s.ok()) {
