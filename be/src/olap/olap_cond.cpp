...
void CondColumn::merge_cond(const CondColumn& cond_col) {
    ...
    for (auto& cond : _conds) {
        for (const auto& old_cond : cond_col._conds) {
            if ((cond->op == old_cond->op) ||
                (cond->op == OP_NOT_IN && old_cond->op == OP_NE) ||
                (cond->op == OP_NE && old_cond->op == OP_NOT_IN) ||
                (cond->op == OP_IN && old_cond->op == OP_EQ) ||
                (cond->op == OP_EQ && old_cond->op == OP_IN) ||
                (cond->op == OP_LT && old_cond->op == OP_LE) ||
                (cond->op == OP_LE && old_cond->op == OP_LT) ||
                (cond->op == OP_GT && old_cond->op == OP_GE) ||
                (cond->op == OP_GE && old_cond->op == OP_GT)) {
                CHECK_EQ(cond->union_cond(*old_cond), OLAP_SUCCESS);
                break;
            }
        }
    }
...
bool Conditions::delete_conditions_eval(const RowCursor& row) const {
    ...
    VLOG(3) << "Row meets the delete conditions. "
            << "condition_count=" << _sorted_conds.size() << ", row=" << row.to_string();
    return true;
}
...
bool Conditions::rowset_pruning_filter(const std::vector<KeyRange>& zone_maps) const {
    ...
    for (auto& cond_it : _sorted_conds) {
        if (_cond_column_is_key_or_duplicate(cond_it)) {
            if (cond_it->col_index() < zone_maps.size() &&
                !cond_it->eval(zone_maps.at(cond_it->col_index()))) {
                return true;
            }
        }
    }
