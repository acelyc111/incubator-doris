...
Status FragmentMgr::exec_plan_fragment(
    ...
    // TODO(yingchun): make it as a configuration, we can disable to create a new thread to process it
    if (use_pool) {
        ...
        int ret = pthread_create(&id,
                       nullptr,
                       fragment_executor,
                       // TODO(yingchun): why not execute it directly, but new a PriorityThreadPool::WorkFunction
