...
void add_default_path_handlers(WebPageHandler* web_page_handler, MemTracker* process_mem_tracker) {
    web_page_handler->register_page("/logs", logs_handler, true /* is_on_nav_bar */);
    web_page_handler->register_page("/varz", config_handler, true /* is_on_nav_bar */);
    web_page_handler->register_page(
            "/memz", boost::bind<void>(&mem_usage_handler, process_mem_tracker, _1, _2), true);
}
