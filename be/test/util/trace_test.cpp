...
TEST_F(TraceTest, TestBasic) {
  ...
  std::cout << t->DumpToString(Trace::NO_FLAGS);
  std::cout << "====================" << std::endl;
  std::cout << t->DumpToString(Trace::INCLUDE_TIME_DELTAS);
  std::cout << "====================" << std::endl;
  std::cout << t->DumpToString(Trace::INCLUDE_METRICS);
  std::cout << "====================" << std::endl;
  std::cout << t->DumpToString(Trace::INCLUDE_ALL);
  std::cout << "====================" << std::endl;
...
TEST_F(TraceTest, TestChildTrace) {
  scoped_refptr<Trace> traceA(new Trace);
  scoped_refptr<Trace> traceB(new Trace);
  scoped_refptr<Trace> traceB1(new Trace);
  scoped_refptr<Trace> traceC(new Trace);
  ADOPT_TRACE(traceA.get());
  traceA->AddChildTrace("child", traceB.get());
  traceA->AddChildTrace("child1", traceB1.get());
  traceB->AddChildTrace("child of child", traceC.get());
  TRACE("hello from traceA");
  {
    ADOPT_TRACE(traceB.get());
    TRACE("hello from traceB");
    {
      ADOPT_TRACE(traceB1.get());
      TRACE("hello from traceB1");
    }
    {
      ADOPT_TRACE(traceC.get());
      TRACE("hello from traceC");
    }
  }
  std::cout << traceA->DumpToString(Trace::INCLUDE_ALL);
  std::cout << "====================" << std::endl;
  std::cout << traceB->DumpToString(Trace::INCLUDE_ALL);
  std::cout << "====================" << std::endl;
//  EXPECT_EQ("XXXX XX:XX:XX.XXXXXX trace_test.cpp:XXX] hello from traceA\n"
//            "Related trace 'child':\n"
